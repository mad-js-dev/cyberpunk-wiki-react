class SelectionMenu extends HTMLElement {
  ...
  _renderItems(items, level = 0) {
    ...
    items.forEach((item) => {
      ...
      if (hasChildren) {
        ...
        // Set the label prop on the collapsible item
        if (item.label) {
          li.setAttribute('label', item.label);
        }
        
        // Set up expand/collapse state
  ...
  _onItemClick(event) {
    // Handle click on leaf items
    const leafNode = event.target.closest(".menu-item__leaf");
    if (leafNode) {
      event.stopPropagation();
      const itemId = leafNode.closest('li').getAttribute("data-id");
      if (itemId) {
        this._handleItemSelection(itemId);
      }
      return;
    }
    
    // Handle click on collapsible item headers
    const header = event.target.closest("collapsible-item");
    if (header) {
      event.stopPropagation();
      const itemId = header.getAttribute("data-id");
      if (itemId) {
        this._handleItemSelection(itemId);
      }
      return;
    ...
    // Handle toggle click (arrow icon)
    const toggleButton = event.target.closest("button[part='toggle']");
    if (toggleButton) {
      const itemElement = toggleButton.closest("li[is='collapsible-item']");
      if (itemElement) {
        event.stopPropagation();
  ...
  _handleItemSelection(itemId) {
    ...
    // Find the selected item in the items tree
    ...
    const selectedItem = findItem(this._items, itemId);
    ...
    // Update the selected attribute for form association
  ...
  _updateSelectedState() {
    ...
    // Set selection on the currently selected item
    if (this._selectedId) {
